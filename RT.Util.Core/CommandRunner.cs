...
///     heavily towards executing console programs, batch files and console scripts, but can also execute built-in commands
///     and non-console programs. Because the command goes through cmd.exe, the PATH, PATHEXT and file associations are all
///     automatically taken care of.</summary>
/// <remarks>
///     <para>
///         When the command completes, whether by exiting naturally or by being aborted, the <see cref="State"/> changes
///         first. Then the <see cref="CommandEnded"/> event fires. Finally, the <see cref="EndedWaitHandle"/> is signalled.
///         By the time the <see cref="State"/> has changed to <see cref="CommandRunnerState.Exited"/>, it is guaranteed that
///         all the output has been processed and the <see cref="ExitCode"/> can be retrieved.</para>
///     <para>
...
public class CommandRunner
{
    ...
    /// <summary>Raised whenever the command has produced new output on stdout (but no more often than about once in 50ms).</summary>
    public event Action<byte[]> StdoutData;
    /// <summary>Raised whenever the command has produced new output on stderr (but no more often than about once in 50ms).</summary>
    ...
    ///     cref="StdoutData"/>, however utf8-encoded text is guaranteed to correctly handle the possibility that part of a
    ///     character's encoding has not been output yet. Other encodings are not supported.</summary>
    ...
    /// <summary>
    ///     Specifies whether the entire stdout output should be captured. This can consume large amounts of memory, and so
    ...
    /// <summary>
    ///     Specifies whether the entire stderr output should be captured. This can consume large amounts of memory, and so
    ///     defaults to <c>false</c>.</summary>
    ...
    /// <summary>
    ///     Exposes a waitable flag indicating whether the command has ended, either by exiting naturally or by being aborted.
    ///     See also Remarks on <see cref="CommandRunner"/>.</summary>
    /// <remarks>
    ///     Calling Set or Reset will result in corruption and undefined behaviour. This should be a read-only waitable flag,
    ///     but there is no built-in class</remarks>
    public WaitHandle EndedWaitHandle { get { return _ended.WaitHandle; } }

    /// <summary>
    ///     Indicates the current state of the runner. Some properties are only readable and/or writable in specific states.
    ///     Note that this property is not guaranteed to reflect whether the actual command is running or not, or whether a
    ///     call to <see cref="Abort"/> resulted in the command of being terminated; rather, this is the logical state of the
    ///     <see cref="CommandRunner"/> itself.</summary>
    ...
    /// <summary>
    ///     The raw command to be executed, as a single string. Any command supported by cmd.exe is permitted. See also <see
    ///     cref="SetCommand(string[])"/>, which simplifies running commands with spaces and/or arguments. Once the command
    ///     has been started, this property becomes read-only and indicates the value in effect at the time of starting. See
    ///     Remarks.</summary>
    ...
    /// <summary>
    ///     Overrides for the environment variables to be set for the command process. These overrides are in addition to the
    ///     variables inherited from the current process.</summary>
    ...
    ///     parsing them with standard Windows tools will receive the exact strings passed in. Null values are allowed and are
    ///     filtered out. The first value is escaped as a path to an executable and all others as parameters. Thus it is not
    ///     possible to use this method to construct arguments that are handled specially by cmd.exe such as redirects, pipes
    ///     and environment variable expansions.</summary>
    /// <remarks>
    ///     Example: <c>SetCommand(new[] { @"C:\Program Files\Foo\Foo.exe", "-f", @"C:\Some Path\file.txt" });</c>. See also
    ///     <see cref="ArgsToCmdExeLine(IEnumerable{string})"/> which performs the same exact escaping but returns the value
    ///     instead.</remarks>
    ...
    /// <remarks>
    ///     Example: <c>SetCommand(new[] { @"C:\Program Files\Foo\Foo.exe", "-f", @"C:\Some Path\file.txt" });</c>. See also
    ///     <see cref="ArgsToCmdExeLine(string[])"/> which performs the same exact escaping but returns the value instead.</remarks>
...
#pragma warning restore CA1416
    ...
    ///     an executable or a script, and the rest as arguments.</summary>
    ...
    ///     process name + arguments to cmd.exe then further escaping is required, to counteract cmd.exe's interpretation of
    ///     additional special characters. See <see cref="EscapeCmdExeMetachars"/>.</remarks>
    ...
    /// <summary>
    ///     Aborts the command by killing the process and all its children, if any. Throws if the command has not been started
    ///     yet. Does nothing if the command has already ended. See Remarks.</summary>
    /// <remarks>
    ///     It is theoretically possible that this method will fail to terminate a child, if called at exactly the wrong time
    ///     while a child process is being spawned. To avoid this possibility, pause the process tree first by calling <see
    ///     cref="Pause"/> (but see its remarks too).</remarks>
    public void Abort()
    {
        if (State == CommandRunnerState.NotStarted)
    ...
    ///     Throws if the command hasn't been started or has been aborted. Does nothing if the command has exited. If called
    ///     when already paused, will make sure the command is paused for at least the specified duration, but will not
    ///     shorten the resume timer. See Remarks.</summary>
    /// <param name="duration">
    ...
    /// <remarks>
    ///     It is theoretically possible for this to fail to suspend all threads in all processes, if a new thread is created
    ///     while the threads are being suspended. The current code does not address this scenario, but this can be fixed, if
    ///     necessary.</remarks>
...
#pragma warning restore CA1806
    ...
    ///     cref="SetCommand(string[])"/> for further information. See <see cref="RunRaw"/> for an alternative way to specify
    ///     the command.</param>
    /// <remarks>
    ///     <para>
    ///         The default options are as follows: 0 is the only exit code indicating success; print the command output to
    ///         the console as-is.</para>
    ///     <para>
...
public enum CommandRunnerState
{
    ...
    NotStarted,
    /// <summary>
    ///     The command has started and has neither been aborted yet nor has exited on its own. This state does not guarantee
    ///     that the process in which the command is executed has already started, nor that it has not yet terminated.</summary>
    Started,
    /// <summary>The command has exited on its own, without the runner terminating it.</summary>
    Exited,
    /// <summary>
    ///     The command has been terminated by the runner. This state does not guarantee that the process in which the command
    ///     is executed has already been killed.</summary>
    Aborted,
}
...
public class FluidCommandRunner
{
    ...
    /// <summary>
    ///     Configures the runner to suppress all output. This invocation will print nothing to the console. If unspecified,
    ///     the command's stdout output is relayed to the console as-is, while stderr is relayed in red.</summary>
    ...
    ///     entire command is printed before running it. When the command completes, its success/failure status is printed,
    ///     along with its run time and exit code.</summary>
    ...
    /// <summary>
    ///     Invokes the command, blocking until the command finishes. On success, returns the raw output of the command. If
    ///     the command fails, throws a <see cref="CommandRunnerFailedException"/>. See Remarks.</summary>
    /// <remarks>
    ///     Output options such as <see cref="OutputAugmented"/> do not affect the data returned; they influence only how the
    ///     output is relayed to the console. This method ignores all stderr output.</remarks>
    [DebuggerHidden]
    ...
    /// <summary>
    ///     Invokes the command, blocking until the command finishes. Returns the command's exit code. Does not throw if the
