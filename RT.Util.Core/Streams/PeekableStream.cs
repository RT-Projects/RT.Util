...
/// <summary>
///     Implements a stream that exposes methods to transparently peek at the bytes that would be read by a call to <see
///     cref="Read"/>, without affecting the actual outcome of future calls to standard Stream methods. See Remarks for
...
///     <para>
///         Peeking will cause reads in the underlying stream, but this class ensures that the data is read only once, in
///         order to be compatible with pure (read-once, non-seekable) streams.</para>
///     <para>
///         The class maintains that any operations on the streams returned by <see cref="GetPeekStream"/> do not modify the
///         outcome of any other operations performed on this stream, including the value of <see cref="Position"/>, the bytes
///         read by <see cref="Read"/>, or the effects of <see cref="Write"/>. One visible side effect may be the change in
///         chunk size returned by a single call to <see cref="Read"/>. This assumes that if the underlying stream is seekable
///         then nothing else changes the data stored in it.</para>
///     <para>
...
public class PeekableStream : Stream
{
    ...
    /// <summary>
    ///     Gets or sets the current position in the stream, if supported by the underlying stream. Note that seeking causes
    ///     all peek streams to be reset so as to resume peeking from the point seeked to. Note also that getting the current
    ///     position requires a small amount of computation and should be used with care in tight loops.</summary>
    ...
    /// <summary>
    ///     Seeks to the specified position in the underlying stream, if the underlying stream supports it. Note that seeking
    ...
    /// <summary>
    ///     Writes data to the underlying stream. See Remarks for notes concerning seekable underlying streams. If the
    ///     underlying stream is not seekable, writes are assumed to be separate to reads and thus peeks, so the peek streams
    ///     aren't touched.</summary>
    /// <remarks>
    ///     <para>
    ///         Notes for seekable underlying streams:</para>
    ///     <para>
    ///         The write will occur at the current position regardless of how much the peek streams have peeked.</para>
    ///     <para>
    ...
    /// <summary>
    ///     Reads data from the stream into the specified buffer. Advances all peek streams that got overtaken by the new
    ///     stream position so as to continue reading from //</summary>
    /// <returns>
    ///     Number of bytes actually read. Zero if called on a stream that has already ended, or if <paramref name="count"/>
    ///     was zero.</returns>
    ...
    /// <summary>
    ///     Behaves like Read, except that the bytes are discarded.</summary>
    /// <param name="count">
    ///     Maximum number of bytes to skip.</param>
    /// <returns>
    ///     Number of bytes actually skipped. Zero if called on a stream that has already ended, or if <paramref
    ...
    /// <summary>
    ///     Skips the specified number of bytes in the current stream, and throws <see cref="EndOfStreamException"/> if the
    ///     end of the stream is reached early.</summary>
    /// <param name="count">
    ///     Number of bytes to skip.</param>
    ...
    /// <summary>
    ///     <para>
    ///         Creates and returns a new peek stream linked to this stream. Reading from the returned stream allows peeking
    ///         at the bytes ahead of the current position in this stream, without changing the outcome of future calls to any
    ///         methods on this stream.</para>
    ///     <para>
    ///         The returned stream must be disposed of when done, since outstanding undisposed peek streams have a slight
    ...
    /// <summary>
    ///     Reads at most the specified number of bytes into the peek buffers and updates all peek streams which had run out
    ///     of peek buffers. Doesn't change anything if the stream has ended or <paramref name="count"/> was zero.</summary>
    ...
    /// <summary>
    ///     Removes exactly the specified number of bytes from the start of the peek buffers. Advances all peek streams that
    ///     got overtaken to resume peeking from the next unremoved byte, if available. If there aren't enough bytes left in
    ///     the peek buffers, removes all the available bytes.</summary>
    /// <returns>
    ///     The actual number of bytes removed (which is always the requested count unless the buffers became completely
    ///     emptied).</returns>
    ...
    ///     is unaffected when this stream is used to peek ahead of the current position in the parent stream.</summary>
    public class PeekStream : Stream
    {
        ...
        /// <summary>
        ///     Initially true. Whenever the parent stream's position overtakes the position of this stream, changes to false.
        ///     In this state, attempts to read will throw an <see cref="InvalidOperationException"/> and the stream becomes
        ///     useless.</summary>
        public bool IsValid { get; internal set; }

        /// <summary>
        ///     Peeks at most the specified number of bytes from the parent stream. Because the peek stream maintains its own
        ///     position, subsequent calls to this method will peek further and further into the parent stream. Note however
        ///     that direct operations on the parent's underlying stream can break the sequence, and that some calls on the
        ///     parent stream will reset this position. All such calls are documented to this effect.</summary>
        /// <returns>
        ///     The actual number of bytes peeked. Zero if called when peeked all the way to the end of the parent stream, or
        ///     if <paramref name="count"/> was zero.</returns>
        public override int Read(byte[] buffer, int offset, int count)
        {
            if (!IsValid)
