...
public abstract class RTSmtpSettings : IClassifyObjectProcessor
{
    ...
    /// <summary>
    ///     SMTP username for login - for "me@example.com" this is typically "me" or "me@example.com", but can be anything.</summary>
    public string Username = "example_user";
    /// <summary>Unencrypted password to be automatically encrypted by Classify whenever the settings are loaded or saved.</summary>
...
public sealed class RTSmtpClient : IDisposable
{
    ...
    /// <param name="headers">
    ///     Zero or more headers. Headers must be in the correct format, including escaping, and excluding the linebreak that
    ///     separates headers (but including any line breaks required within the header). This method does not validate the
    ///     headers in any way, and does not include From, To, Date or any other headers unless explicitly passed in through
    ///     this argument.</param>
    /// <param name="bodyPlain">
    ///     Plain-text version of the e-mail.</param>
    /// <param name="bodyHtml">
...
/// <summary>
///     Implements a simple interface for sending an email that shares a global repository of SMTP settings. This repository
///     is stored using <see cref="SettingsFileXml{TSettings}"/> using the name "RT.Emailer". The repository contains a list
...
public static class Emailer
{
    ...
    ///     an exception will be thrown, describing what the user needs to do - though this requires a pretty technical user.</summary>
    /// <param name="to">
    ...
    /// <param name="account">
    ///     The name of one of the RT.Emailer accounts to use (case-sensitive). If null or not defined, will fall back to exe
    ///     name, then the Default Account setting, and then any defined account, in this order.</param>
    /// <param name="fromName">
