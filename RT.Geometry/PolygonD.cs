...
public sealed class PolygonD
{
    ...
    public bool ContainsPoint(PointD point)
    {
        ...
        while (true)
        {
            ...
            if (i > 0) prev = _vertices[i - 1];
            else prev = _vertices[len - 1];
        ...
        if (isAbove == startingAbove) return val == 0 ? false : true;
        if (i == len) i = 0;
    ...
    private static double crossProduct3(PointD pt1, PointD pt2, PointD pt)
    {
        ...
        return double.FusedMultiplyAdd(pt2.X - pt1.X, pt.Y - pt2.Y, (pt1.Y - pt2.Y /*negated*/) * (pt.X - pt2.X));
#else
        return (pt2.X - pt1.X) * (pt.Y - pt2.Y) - (pt2.Y - pt1.Y) * (pt.X - pt2.X);
#endif
    }
