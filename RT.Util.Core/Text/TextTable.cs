...
public sealed class TextTable
{
    ...
    /// <summary>
    ///     Gets or sets the maximum width of the table, including all column spacing. If <see cref="UseFullWidth"/> is false,
    ///     the table may be narrower. If this is null, the table width depends on which method is used to generate it.</summary>
    ...
    /// <summary>
    ///     Gets or sets a value indicating the number of rows from the top that are considered table headers. The only effect
    ///     of this is that the horizontal rule (if any) after the header rows is rendered using '=' characters instead of
    ///     '-'.</summary>
    public int HeaderRows { get; set; }
    /// <summary>
    ///     If true, the table will be expanded to fill the <see cref="MaxWidth"/>. If false, the table will fill the whole
    ///     width only if any cells need to be word-wrapped.</summary>
    ...
    ///     <paramref name="content"/>. The cell is word-wrapped only if doing so is necessary to fit all no-wrap cells into
    ///     the table's total width. If false, the cell is automatically word-wrapped to optimise the table's layout.</param>
    ...
    /// <summary>
    ///     Places content at (<see cref="CurCol"/>, <see cref="CurRow"/>) and advances <see cref="CurCol"/> by <paramref
    ///     name="colSpan"/>. See also <see cref="FinishRow()"/>.</summary>
    /// <param name="content">
    ///     The content to place.</param>
    /// <param name="colSpan">
    ///     The number of columns to span.</param>
    ...
    /// <summary>
    ///     Ends the row that was being added to by <see cref="AddCell"/> and begins the next row at column 0.</summary>
    /// <param name="backgroundColor">
    ///     Background color of the row being finished. See <see cref="SetRowBackground"/>.</param>
