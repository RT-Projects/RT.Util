...
/// <summary>
///     <para>
///         Wraps a <see cref="System.Drawing.Graphics"/> to provide a hopefully more convenient interface. The major bits of
...
#pragma warning disable 1591    // Missing XML comment for publicly visible type or member
    ...
    ///     size is used for "set viewport" methods which do not take a screen location: they assume the relevant screen edge
    ///     is meant instead.</summary>
    ...
    /// <summary>Creates an instance using the specified graphics and screenSize. See <see cref="ScreenSize"/> for more info.</summary>
    ...
    /// <summary>
    ///     Sets the scaling and offset so that the world coordinate "leftWX" corresponds to the leftmost coordinate on the
    ///     screen, world "topWY" to the topmost screen coordinate, etc. If "maintainAspect" is "true", ensures that X and Y
    ///     scaling is the same, by making one of the axes show more than implied by the arguments passed in.</summary>
    ...
    /// <summary>
    ///     For internal use only. Given two world coordinates, one known to be smaller than the other one, returns the one
    ///     that would be higher on the screen, converted to screen coordinates.</summary>
    ...
    /// <summary>
    ///     Draws a rectangle using the specified pen. "xMin" and "yMin" specify the corner that has the smallest coordinates,
    ///     so the resulting rectangle will be on coordinates xMin, yMin, xMin+width, yMin+height.</summary>
    ...
    /// <summary>
    ///     Draws a "pie" using the specified pen. A pie is a circular arc whose endpoints are connected to the centre with
    ...
    /// <summary>Draws text using the specified font and brush. The text's bounding box is centered on the specified point.</summary>
    ...
    /// <summary>
    ///     Draws text using the default font and the specified brush. The text's bounding box is centered on the specified
    ///     point.</summary>
    ...
    public GraphicsPath MakeRoundedHorzVertPath(PointD position, double radius, IEnumerable<double> lengths)
    {
        ...
        bool horz = true;
        var gp = new GraphicsPath();
        var prevpair = (lengths.Last(), lengths.First());
