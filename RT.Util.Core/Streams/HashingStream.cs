...
/// <summary>Calculates MD5 checksum of all values that are read/written via this stream.</summary>
public sealed class HashingStream : Stream
{
    ...
    /// <summary>
    ///     Initialises a hash calculation stream, with the specified stream as the underlying stream.</summary>
    /// <param name="stream">
    ///     The underlying stream.</param>
    /// <param name="hasher">
    ///     The hash algorithm to use. For example, <c>System.Security.Cryptography.MD5.Create("MD5")</c>.</param>
...
#pragma warning restore 1591    // Missing XML comment for publicly visible type or member
    ...
    /// <summary>Reads data from the underlying stream. Updates the hash with the bytes read.</summary>
    ...
    /// <summary>Writes data to the underlying stream. Updates the hash with the bytes written.</summary>
    ...
    /// <summary>
    ///     Retrieves the hash of all data that has been read/written through this stream so far. Due to the implementation of
    ///     the underlying hash algorithm this must be called only after all data has been hashed. No further calls to
    ///     Read/Write are allowed after a single call to this.</summary>
    public byte[] Hash
    {
        get
        {
