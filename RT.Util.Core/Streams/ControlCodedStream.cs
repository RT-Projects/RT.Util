...
/// <summary>
///     Implements a stream which enables control codes to be read from or written to the underlying stream at certain points.
///     This stream handles encoding and decoding the control codes and ensuring they are distinguishable from the payload
///     data. See Remarks.</summary>
/// <remarks>
...
public class ControlCodedStream : Stream
{
    ...
    /// <summary>
    ///     Writes the specified control code to the stream.</summary>
    /// <param name="code">
    ///     The code to write. Valid values are 0..254; 255 is reserved.</param>
    ...
    ///     returns <c>-1</c>. Otherwise returns the code read. Throws <see cref="EndOfStreamException"/> if the stream ends
    ...
    /// <summary>
    ///     Determines if the stream has ended. Note that this method may block, potentially indefinitely, in cases where it
    ///     isn't known yet if the stream has ended (for example, a NetworkStream for a socket that is idle but not closed).</summary>
    public bool IsEnded
    {
        get
        {
    ...
    /// <summary>
    ///     Reads up to <paramref name="count"/> bytes into the buffer from the underlying stream. If a control code is
    ///     encountered before any data has been read, will throw an <see cref="InvalidOperationException"/>. If some data has
    ///     already been read upon encountering a control code, will stop and return all the data read up to the control code.</summary>
    /// <returns>
    ///     The number of bytes actually read. Returns zero if the stream was ended when the read started, or if <paramref
