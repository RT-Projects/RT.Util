...
public class HClient
{
    ...
    ///     properties, rather than setting values via the Headers collection. This class unrestricts some of those headers,
    ///     specifically: Accept, Host, Referer, TransferEncoding, UserAgent. Some of the restricted headers are only
    ///     supported partially, indirectly or via properties, specifically: Connection, ContentType, ContentLength, Expect.
    ///     The remaining restricted headers cannot currently be modified at all, specifically: Date, IfModifiedSince,
    ///     ProxyConnection, Range</remarks>
    public WebHeaderCollection ReqHeaders { get; set; }

    /// <summary>
    ///     A shorthand for getting and setting ReqHeaders[HttpRequestHeader.Accept]. Defaults to what a recent version of US
    ///     Firefox might send.</summary>
    ...
    /// <summary>
    ///     A shorthand for getting and setting ReqHeaders[HttpRequestHeader.UserAgent] Defaults to what a recent version of
    ...
    /// <summary>
    ///     Specifies how long to wait for the server to respond to the request before throwing an exception. Defaults to 10
    ///     seconds.</summary>
    public TimeSpan Timeout { get; set; }
    /// <summary>
    ///     Specifies whether the receipt of a redirect should automatically generate a new request for the new URL. Defaults
    ///     to false.</summary>
    ...
    /// <summary>
    ///     Specifies whether to use Expect: 100 (continue) for post requests, thus querying the server for whether it wants
    ///     to accept the post request, before sending the request body. Defaults to false. If true, the entire interaction is
    ///     handled transparently.</summary>
    ...
    /// <param name="args">
    ///     Query parameters to add to the end of the URL in the usual <c>?k1=v1&amp;k2=v2&amp;...</c> format. Null values are
    ///     allowed and ignored.</param>
    /// <returns>
...
public class HArg
{
    ...
    ///                     and <see cref="FileContent"/> are non-<c>null</c>.</description></item></list></description></item></list></remarks>
