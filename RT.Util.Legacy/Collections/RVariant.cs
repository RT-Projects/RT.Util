...
/// <summary>Determines what kind of an RVariant we're dealing with. See documentation for individual items.</summary>
public enum RVariantKind
{
    ...
    ///     being accessed in a certain way, e.g. perfoming string indexing will turn into a Dict.</summary>
    Stub,

    /// <summary>This RVariant stores a char, a string, a DateTime or one of the integer types.</summary>
    Value,

    /// <summary>This RVariant is a List, holding other RVariants accessible via an integer index.</summary>
    List,

    /// <summary>This RVariant is a Dict, holding other RVariants accessible via a string key.</summary>
    Dict,
}
...
///                                  // save to an XML file</code>
///     <para>
///         Properties that aid debugging:</para>
///     <code>
...
///         v.Value     - for Value variants, returns the value stored.</code></summary>
public sealed class RVariant : IEquatable<RVariant>, ICloneable, ICollection<RVariant>
{
    ...
    ///     has not been determined yet.</summary>
    public RVariantKind Kind { get { return _kind; } }

    /// <summary>
    ///     Defines what kind of a node this object represents. The kind may never be changed with a single exception: a
    ///     change from the Stub kind to any other kind is permitted. Hence the ONLY legal way to set this field internally is
    ///     via the <see cref="assumeKind"/> method, and there is no explicit way to change the Kind from outside of the
    ///     RVariant.</summary>
    ...
    ///     into parts (since arbitrary Dict keys are allowed and are not escaped before use in this property)</summary>
    public string FullPath { get { return _path; } }

    /// <summary>
    ///     Same as the <see cref="FullPath"/> property except that it never returns a null. If the path is null returns the
    ///     string "&lt;null-path&gt;" instead. Note that, just like the FullPath property, this is only meant for debugging.</summary>
    public string FullPathNoNull { get { return _path == null ? "<null-path>" : _path; } }

    /// <summary>
    ///     Stores the full path to this node. This field is fully maintained by the class upon all operations which alter it,
    ///     such as assignments. The path is null for any RVariant which is not part of a tree. List and Dict are always
    ///     considered to be part of a tree.</summary>
    ...
    ///     non-Value kind RVariant.</summary>
    public object Value
    {
        get
        {
    ...
    /// <summary>
    ///     Stores the value represented by those RVariants whose Kind is Value. RVariant always stores the value as-is, i.e.
    ///     if assigned an sbyte this field will hold a boxed sbyte. This field is non-null if and only if the kind is Value.
    ///     It defaults to "" whenever a Stub gets automatically promoted into a Value.</summary>
    private object _value;

    /// <summary>
    ///     Stores a list of RVariants if the Kind is List. This field is non-null if and only if the Kind is List. The
    ///     variants stored in this list are never null - they are at least Stubs.</summary>
    private List<RVariant> _list;

    /// <summary>
    ///     Stores a dict of RVariants if the Kind is Dict. This field is non-null if and only if the Kind is Dict. The
    ///     variants stored in this list are never null - they are at least Stubs.</summary>
    ...
    ///     a List or a Dict by performing an operation on it. E.g. indexing [] with an integer will turn it into a List.</summary>
    public RVariant()
    {
    }

    /// <summary>
    ///     Even though this constructor is private, the user can easily invoke it via one of the defined implicit casts. This
    ///     ensures that only the supported types can be stored.</summary>
    ...
    /// <summary>
    ///     Creates a RVariant from the specified XmlDocument. Does not tolerate errors - will throw an exception if the XML
    ///     is not a valid representation of a RVariant.</summary>
    ...
    /// <summary>
    ///     Creates a RVariant from the specified XmlDocument. Does not tolerate errors - will throw an exception if the XML
    ///     is not a valid representation of a RVariant. The name of the root element will be stored in the "rootNodeName"
    ///     parameter.</summary>
    ...
    /// <summary>
    ///     Recreates a RVariant from the specified XmlElement. Can be used to store several RVariants in a single
    ///     XmlDocument.</summary>
...
#pragma warning disable CS1574 // XML comment has cref attribute that could not be resolved - .NET gets confused by RT.Json's inclusion of a private copy of ExactConvert with #if
    ...
    ///     <see cref="ExactConvert.ToString"/>. Otherwise returns a string showing the Path and the Kind of this value.</summary>
#pragma warning restore CS1574
    ...
    /// <summary>
    ///     Changes the Kind of this RVariant. Throws an Internal Error exception if the change is not valid - callers must
    ///     check themselves and throw a friendlier exception. Upon changing the kind from a Stub to another Kind will
    ///     initialise the fields relevant to that kind.</summary>
    ...
    /// <summary>
    ///     Wipes whatever was in this variant and overwrites with a copy of the supplied variant. Note that all references to
    ///     this variant remain valid (which is the whole point of this method, vs. a Clone() method, which creates a _new_
    ///     variant).
    /// 
    ///      Copying from "null" will throw a NullReferenceException.</summary>
    ...
    /// <summary>Creates a deep copy of this RVariant.</summary>
    public object Clone()
    {
    ...
    ///     slow - it's probably a bad idea to use RVariant as a dictionary key...</summary>
    ...
    ///     two. Otherwise compares the Value stored in this RVariant with the specified object.</summary>
    ...
    /// <summary>
    ///     Compares RVariant to another object. If the RVariant and the object are not null, uses <see
    ///     cref="Equals(object)"/> to do the comparison.</summary>
    ...
    /// <summary>The opposite of <see cref="operator=="/>.</summary>
    public static bool operator !=(RVariant a, object b)
    {
        if ((object) a == null && (object) b == null)
            return false;
    ...
    /// <summary>
    ///     Called before accessing a Dict element, to ensure that this node is a Dict kind and to add the element if it
    ...
    /// <summary>
    ///     Called before accessing a List element, to ensure that this node is a List kind and to add the element if it
    ///     doesn't exist yet.</summary>
    ...
    ///     attempted on a node which is already a List or a Value.</summary>
    ...
    ///     attempted on a node which is already a Dict or a Value.</summary>
    ...
    ///     which will return true only if the variant /stuff/value is defined.</summary>
    public bool Exists
    {
        get
        {
    ...
    ///     XmlElement will not be altered.</summary>
    ...
    /// <summary>Loads an XML file into this RVariant, deleting all existing values and adding new ones as appropriate.</summary>
    ...
    /// <summary>
    ///     Saves this RVariant to an XML file.</summary>
    /// <param name="fileName">
    ///     File name</param>
    /// <param name="rootElementName">
    ///     The name of the root element</param>
    ...
    /// <summary>Gets the number of items stored in this variant. Returns 0 for Stubs and 1 for Values.</summary>
    public int Count
    {
        get
        {
    ...
    /// <summary>Verifies that the value is a List; throws an exception if not. To be used by ICollection implementation only.</summary>
    ...
    /// <summary>
    ///     Makes this RVariant a stub, with no values associated. This is the only legal way to revert the <see cref="Kind"/>
    ///     from non-Stub to Stub.</summary>
    public void Clear()
    {
        _kind = RVariantKind.Stub;
        _value = null;
    ...
    /// <summary>Adds an RVariant to this variant, assuming/making it a List.</summary>
    ...
    /// <summary>Iterates over the items in this variant, assuming/making it a List.</summary>
    ...
    /// <summary>Always returns false since RVariants are never read-only.</summary>
...
/// <summary>All RVariant exceptions derive from this one.</summary>
public class RVariantException : Exception
{
    /// <summary>Should only be used by constructors which initialise the error message in the constructor body.</summary>
...
/// <summary>
///     Indicates that there was an error while attempting to convert a Value kind variant from one type to another. E.g.
///     converting "hi" to int.</summary>
...
/// <summary>
///     Indicates that a variant was not found where expected. This is only thrown from the .OrNotFound series of methods.</summary>
...
/// <summary>Indicates that RVariant encountered an error while converting XML to variant.</summary>
