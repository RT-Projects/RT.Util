...
/// <summary>
///     A window which has all the proper minimize/restore methods, and which remembers its position and size between
///     instances of the application. The size/position/state values are automatically reflected in the settings object
///     whenever the user resizes or moves the window. If XAML supplies an initial size, that size will be used whenever no
///     previous size is available (such as first run).</summary>
public class ManagedWindow : Window
{
    ...
    /// <summary>
    ///     We need a default constructor for the WPF designer to work. Don't invoke this or else the settings won't work.</summary>
    ...
    /// <summary>
    ///     To make the window maximized by default (i.e. on first run, before the user has had a chance to move/resize the
    ///     window), override this property and return true. This property is read from the window constructor. Do not use
    ///     XAML to set Maximized to true because <see cref="ManagedWindow"/> will not be able to override that without
    ///     flicker (and thus doesn't support that at all).</summary>
