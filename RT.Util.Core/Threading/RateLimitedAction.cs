...
public class RateLimitedAction
{
    ...
    ///     the action is complete. If not, returns very quickly without blocking. If another thread is currently executing
    ///     the action, returns very quickly without blocking.</summary>
    /// <param name="action">
    ...
    /// <summary>
    ///     Makes it so that the next action will occur no earlier than the <see cref="MinActionInterval"/>. If action is
    ///     already in progress, it will complete as usual.</summary>
    ...
    /// <summary>
    ///     Makes it so that the action will occur as soon as possible - namely, the next time a thread invokes the <see
    ///     cref="Check"/> method.</summary>
    public void Expire()
    {
