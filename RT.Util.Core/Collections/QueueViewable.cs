...
public sealed class QueueViewable<T> : IEnumerable<T>, ICollection<T>, IList<T>
{
    ...
    /// <summary>
    ///     Accesses the Nth queued item. The next item to be dequeued always has the index 0. The existing items can be both
    ///     read and assigned. No new items can be added using this indexer.</summary>
