...
{
    /// <summary>
    ///     Runs all post-build checks defined in the specified assemblies. This is intended to be run as a post-build event.
    ...
    ///             <para>
    ///                 If your project entails several assemblies, you can specify additional assemblies in the call to <see
    ///                 cref="PostBuildChecker.RunPostBuildChecks"/>. For example, you could specify
    ///                 <c>typeof(SomeTypeInMyLibrary).Assembly</c>.</para></description></item>
    ///         <item><description>
    ///             <para>
    ///                 Add post-build check methods to any type where they may be relevant. For example, you might use code
    ...
    ///                 #endif</code>
    ///             <para>
    ///                 The method is expected to have one parameter of type <see cref="IPostBuildReporter"/>, a return type
    ///                 of void, and it is expected to be static and non-public. Errors and warnings can be reported by
    ///                 calling methods on said <see cref="IPostBuildReporter"/> object. (In the above example,
    ///                 PostBuildChecker will attempt to find a class called Gizmo to link the error message to a location in
    ///                 the source.) Throwing an exception will also report an error.</para></description></item></list></remarks>
