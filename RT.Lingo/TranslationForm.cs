...
/// <summary>
///     Provides a GUI for the user to edit a translation for the application.</summary>
/// <typeparam name="TTranslation">
///     The type containing the <see cref="TrString"/> and <see cref="TrStringNum"/> fields to be translated.</typeparam>
public sealed class TranslationForm<TTranslation> : ManagedForm, ITranslationDialog where TTranslation : TranslationBase, new()
{
    ...
    /// <summary>
    ///     Fires every time the translation is updated on the disk (i.e. when the user clicks either "Save &amp; Close" or
    ...
    /// <param name="settings">
    ///     Settings of the <see cref="TranslationForm&lt;T&gt;"/>.</param>
    /// <param name="icon">
    ///     Application icon to use.</param>
    /// <param name="programTitle">
    ///     Title of the program. Used in the title bar.</param>
    /// <param name="moduleName">
    ///     Used for locating the translation file to be edited under the Translations directory.</param>
    /// <param name="language">
    ///     The language to be edited.</param>
        ...
        : base(settings)
    {
        ...
        var dic = new Dictionary<object, (string groupName, string groupDescription)>();
        foreach (var type in dicPanels.Select(kvp => kvp.Key.GetType()).Distinct())
            foreach (var f in type.GetFields(BindingFlags.Static | BindingFlags.Public))
                foreach (var attr in f.GetCustomAttributes<LingoGroupAttribute>())
                    dic.Add(f.GetValue(null), (attr.GroupName, attr.Description));

        // Create all the list items
        foreach (var kvp in dic)
            if (dicPanels.ContainsKey(kvp.Key))
            {
                var listItem = new TranslationGroupListItem { Label = kvp.Value.groupName, Notes = kvp.Value.groupDescription, TranslationPanels = dicPanels[kvp.Key].ToArray() };
        ...
        ts.Items.Add(new ToolStripMenuItem("&Translation", null,
            _mnuApply = new ToolStripMenuItem("&Save and apply", null, saveAndApply) { ShortcutKeys = Keys.Control | Keys.S, ShortcutKeyDisplayString = "Ctrl+S or F5" },
    ...
    /// <summary>Returns true if the user has made any changes that are currently unsaved.</summary>
    ...
    /// <summary>Closes the translation form without asking the user's confirmation when unsaved changes exist.</summary>
    ...
    private void saveAndApply(object _ = null, EventArgs __ = null)
    {
        if (AnyChanges)
        {
    ...
    private TranslationPanel createTranslationPanel(string notes, object orig, object trans, string label)
    {
        TranslationPanel pnl = (orig is TrString)
            ? (TranslationPanel) new TranslationPanelTrString(notes, (TrString) orig, (TrString) trans, label)
            : (TranslationPanel) new TranslationPanelTrStringNum(notes, (TrStringNum) orig, (TrStringNum) trans, label, _origNumberSystem, _translation.Language.GetNumberSystem());

        pnl.ChangeMade += changeMade;
        pnl.EnterPanel += enterPanel;
        pnl.CtrlUp += ctrlUp;
        pnl.CtrlDown += ctrlDown;
        pnl.CtrlPageUp += ctrlPageUp;
        pnl.CtrlPageDown += ctrlPageDown;
        pnl.PageUp += pageUp;
        pnl.PageDown += pageDown;
        pnl.GroupSwitch += groupSwitch;
    ...
    /// <summary>
    ///     Saves the changes to the translation currently being edited.</summary>
    /// <param name="fireTranslationChanged">
    ///     If true, the <see cref="TranslationChanged"/> event is fired if any changes were made. If false, the event is not
    ///     fired.</param>
    ...
    private abstract class TranslationPanel : TableLayoutPanel
    {
        ...
        public TranslationPanel(string notes, string label, bool outOfDate, bool needOldRow)
            : base()
        {
            ...
            _lblStringCode = new Label { Text = label, UseMnemonic = false, Font = new Font(Font, FontStyle.Bold), AutoSize = true, Margin = new Padding(margin), Anchor = AnchorStyles.Left };
            _lblStringCode.Click += focusTranslationBox;
            ...
            if (!string.IsNullOrEmpty(notes))
            {
                ...
                _lblNotes.Click += focusTranslationBox;
    ...
    private sealed class TranslationPanelTrString : TranslationPanel
    {
            ...
            )
        {
            ...
            _txtTranslation.KeyDown += keyDown;
            _btnAccept.KeyDown += keyDown;
    ...
    private sealed class TranslationPanelTrStringNum : TranslationPanel
    {
        ...
        public TranslationPanelTrStringNum(string notes, TrStringNum orig, TrStringNum trans, string label, NumberSystem origNumberSystem, NumberSystem transNumberSystem)
            : base(notes, label,
                // outOfDate
        ...
        private Panel createTablePanel(TrStringNum str, string[] display, NumberSystem ns, bool textBoxes, int nn, int rows)
        {
            ...
            pnlTranslations.Click += focusTranslationBox;
            for (int i = 0; i < nn; i++)
