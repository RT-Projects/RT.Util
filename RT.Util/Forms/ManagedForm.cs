...
/// <summary>
///     A form which has all the proper minimize/restore methods, and which remembers its position and size between instances
...
public class ManagedForm : Form
{
    ...
    /// <param name="settings">
    ///     An object of type <see cref="ManagedForm.Settings"/> from which the position and size of the form are retrieved,
    ...
    /// <summary>
    ///     Override to alter the window size, maximize state and/or position on first show and screen resolution changes. At
    ///     the time of call, the current position/state/size are already set to the preferred ones, so it is possible to
    ///     change them with respect to the "saved" values. If this method does nothing then the last saved values will be in
    ///     effect.</summary>
    ...
    /// <summary>
    ///     Override to perform an action every time the settings are updated. This can happen multiple times per second as
    ///     the window is being moved or resized.</summary>
