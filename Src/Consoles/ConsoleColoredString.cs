...
namespace RT.Util.Consoles
{
    ...
    public sealed class ConsoleColoredString
    {
        ...
        /// <item><description><c>[.blah]</c> = Surround a whole paragraph with this to add a bullet point and indent the paragraph. Use this to create bulleted lists. The default bullet point character is <c>*</c>; you can use an attribute to specify another one, for example <c>[&lt;-&gt;.blah]</c>.</description></item>
        /// <item><description><c>[+blah]</c> = suppress word-wrapping within a certain stretch of text. In other words, the contents of a <c>[+...]</c> tag are treated as if they were a single word.
        ...
        /// </para>
        /// </remarks>
        public static IEnumerable<ConsoleColoredString> FromCuteNodeWordWrap(CuteNode node, int wrapWidth, int hangingIndent = 0)
        {
            var results = new List<ConsoleColoredString> { ConsoleColoredString.Empty };
            CuteML.WordWrap(node, new cuteWordWrapState(ConsoleColor.Gray, 0), wrapWidth,
